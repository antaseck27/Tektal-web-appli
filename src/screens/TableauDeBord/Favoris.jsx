...
export default function Favoris() {
  ...
  const loadFavorites = async () => {
    ...
    try {
      ...
      if (!res?.ok) {
        throw new Error(res?.message || res?.data?.detail || "Impossible de charger les favoris.");
      }

      const list = normalizeList(res.data).map((p) => ({ ...p, isFavorite: true }));
      setFavorites(list);
    } catch (e) {
      setError(e.message || "Impossible de charger les favoris.");
    }
  ...
  useEffect(() => {
    ...
    // recharge auto quand tu reviens sur l'onglet/fenêtre
  ...
  const onToggleFavorite = async (path) => {
    ...
    try {
      const res = await toggleFavorite(path.id);
      if (!res?.ok) throw new Error(res?.message || "Impossible de mettre à jour le favori.");
    } catch {
      setFavorites(snapshot);
      setError("Impossible de mettre à jour le favori.");
    }
